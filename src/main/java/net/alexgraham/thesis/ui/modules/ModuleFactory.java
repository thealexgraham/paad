...
public class ModuleFactory implements Serializable {
	...
	public static JPanel createSideConnectPanel(ModulePanel module, Connector connector, JComponent component) {
		JPanel insidePanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 1));
	...
	public static void addDoubleParamPanel(ModulePanel module, DoubleParamModel model, JPanel panel, GridBagConstraints c) {
		...
		// Create Components
		// ----------------------
		JLabel paramNameLabel = new JLabel(model.getName().replace("Percent", "%"));
		String valueString = String.format("%5.2f", model.getDoubleValue());
		
		if (model.getDoubleValue() >= 0.0f)
			valueString = " " + valueString;
		JLabel paramValueLabel = new JLabel(valueString);
//		paramNameLabel.setPreferredSize(new Dimension(paramNameLabel.getPreferredSize().width + 20, paramNameLabel.getPreferredSize().height));
		model.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				String valueString = String.format("%5.2f", model.getDoubleValue());
				if (model.getDoubleValue() > 0)
					valueString = " " + valueString;
				paramValueLabel.setText(valueString);
//				App.mainWindow.repaint();
	...
	public static void addIntParamPanel(ModulePanel module, IntParamModel model, JPanel panel, GridBagConstraints c) {
		...
		// -----------------------
		ConnectablePanel leftConnectable = 
				ModuleFactory.getConnectablePanel(module, Location.LEFT, model.getConnector(ConnectorType.PARAM_CHANGE_IN), 0, 5);
